...
const filteredItems = computed(() => {
    ...
    switch (radioFilter.value) {
        case 'today':
            ...
            break;
        case 'past':
            items = items.filter(item => new Date(item.dueAt) < new Date());
            break;
    }
...
<template>
    <div class="bg-white relative border rounded-lg">
        ...
        <table class="w-full text-sm text-left text-gray-500">
            ...
            <tr v-if="filteredItems.length" v-for="item in filteredItems" :key="item.id"
                class="border-b last:border-b-0 even:bg-white odd:bg-slate-50">
            ...
            <tr v-else class="w-full text-center">
                <td colspan="12" class="px-4 py-3 font-medium text-rose-600 italic">
                    <img src="https://cdn-icons-png.flaticon.com/512/11611/11611088.png" alt=""
                        class="m-auto w-10 h-10 animate-bounce object-contain">
                    <p>No Match...</p>
                </td>
            </tr>
        </table>
